...
function showSearchResults() {
  ...
  if (searchQuery) {
    ...
    if (searchQuery.includes("all")) {
      // Show all expenses
      let resultsHTML =
        "<table class='custom-table'><thead><tr><th>Category</th><th>Amount</th><th>Date</th></tr></thead><tbody>";
      expenses.forEach((expense) => {
        resultsHTML += `<tr><td>${
          expense.category
      ...
      const filteredExpenses = expenses.filter(
        (expense) => expense.category.toLowerCase() === searchQuery
      );
      if (filteredExpenses.length > 0) {
        ...
        resultsHTML +=
          "<tr><td colspan='3'>No expenses found for this category.</td></tr>";
      }
      resultsHTML += "</tbody></table>";
      searchResultsContent.innerHTML = resultsHTML;
    }
